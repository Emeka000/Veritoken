...
fn test_get_token_export_reflects_set_compliance_metadata() {
    ...
    let key_entity = soroban_sdk::Symbol::new(&h.env, META_LEGAL_ENTITY);
    let key_isin   = soroban_sdk::Symbol::new(&h.env, META_ISIN);
    h.token.set_compliance_metadata(&h.admin, &key_entity, &String::from_str(&h.env, "Acme Corp"), &h.next_nonce());
    h.token.set_compliance_metadata(&h.admin, &key_isin,   &String::from_str(&h.env, "US0231351067"), &h.next_nonce());
