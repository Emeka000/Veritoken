...
export class InvoiceTokenClient extends BaseContractClient {
  ...
  async isSettled(): Promise<boolean> {
    return this.read<boolean>("is_settled", []);
  }
  ...
  buildIssueXdr(to: string, amount: bigint): string {
    return this.contract
      .call("issue", encodeAddress(to), encodeI128(amount))
      .toXDR("base64");
  }

  /** @deprecated Use `settle()` instead */
  buildSettleXdr(): string {
    return this.contract.call("settle").toXDR("base64");
  }

  /** @deprecated Use `redeem()` instead */
  buildRedeemXdr(from: string, amount: bigint): string {
    return this.contract
      .call("redeem", encodeAddress(from), encodeI128(amount))
      .toXDR("base64");
  }
  ...
  buildUpdateMetaXdr(meta: InvoiceMeta): string {
    return this.contract
      .call("update_meta", encodeInvoiceMeta(meta))
      .toXDR("base64");
  }
}
